...
.. rule :: uid name
    ...
    ``uid`` should have a semantic name. They must be written in lower camel case. Don't follow the ``uid`` by `UID` (like `imageUID`). Moreover, avoid ``uid`` like `myXXXXX` or `customXXXXX`.
...
.. rule :: Core parameters
    ...
    These parameters are given by launcher services and are written in capitals
...
.. rule :: Configuration documentation
    ...
    .. code-block :: xml
        ...
        <!--
        This config open a windows containing the editors to manage the ModelSeries: show/hide reconstructions, change the color, ...
        ...
        - WINDOW_TITLE(optional): title of the window
        -->
