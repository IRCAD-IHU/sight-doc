...
.. warning::
    If the **CMake** version of your distribution is not sufficient (Mint 17 for instance ships only the 2.18 version),
    you can easily grab it on the `Kitware website <https://cmake.org/download/>`_.
    Download the **binary** version (much easier than compiling yourself),
    create a "Software" folder and extract your ".bin" in this folder (i.e. /home/login/software/cmake/).
    Open your bashrc and add the ``bin/`` folder inside your ``PATH`` environment variable:
...
Next, we also need to install specific development libraries for **Qt**.
Please before following the requirements, you must read the next paragraph.
